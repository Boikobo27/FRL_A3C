...
class MComCore(gym.Env):
    ...
    def render(self, mode="human") -> None:
        ...
        fig = plt.figure()
        fx = max(3.0 / 2.0 * 1.25 * self.width / fig.dpi, 10.0)
        fy = max(1.25 * self.height / fig.dpi, 7.0)
        plt.close()
    ...
    def render_simulation(self, ax) -> None:
        ...
        for bs in self.stations.values():
            ...
            ax.scatter(*self.conn_isolines[bs], color="orange", s=3)
            ax.scatter(*self.mb_isolines[bs], color="green", s=3)
...
#         ax.set_ylim([self.utility.lower, self.utility.upper])
    ...
    def render_ues_connected(self, ax) -> None:
        ...
        ax.plot(time, ues_connected, linewidth=3, color="red")
